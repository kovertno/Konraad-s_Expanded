...
// 9.  Debug mode with faster movement speed, when in debug, add a second key that will display only vertices                                              x
// 10. Scoring, lets say that for now in console                                                                                                           x                                                
//

// q - debug mode, e - wireframe mode, c - normals debug mode
...
void PrepTextures(unsigned int* textures, const unsigned int textNum);
void SetStaticLights(Shader& shader, std::vector<glm::vec3>& pointLightPositions);
void SetDynamicLights(Shader& shader);
void Game(GLFWwindow* window, Shader& shader, unsigned int VAO, std::vector<glm::vec3>& cubePositions);
void RenderBoxes(Shader& shader, std::vector<glm::vec3>& cubePositions, unsigned int* textures, unsigned int VAO);
void RenderWalls(Shader& shader, std::vector<glm::vec3>& wallPositions, unsigned int VAO);
...
int main() {
    ...
    unsigned int textures[numText];
    PrepTextures(textures, numText);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        Game(window, cubeShader, boxVAO, cubePositions);

        SetDynamicLights(cubeShader);

        RenderBoxes(cubeShader, cubePositions, textures, boxVAO);
...
void ProcessInput(GLFWwindow* window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS && !cPressed) {
        isDebugColorMode = !isDebugColorMode;
        cPressed = true;
    }
...
void PrepTextures(unsigned int* textures, const unsigned int numText) {
    // load and create textures
    glGenTextures(numText, textures);
...
void SetStaticLights(Shader& shader, std::vector<glm::vec3>& pointLightPositions) {
    ...
    shader.setFloat("textureMaterial.shininess", 32.0f);
    ...
    for (int i = 0; i < NUM_P_LIGHT; i++) {
        ...
        shader.setFloat("pointLight[" + number + "].constant", 1.0f);
        shader.setFloat("pointLight[" + number + "].linear", 0.09f);
        shader.setFloat("pointLight[" + number + "].quadratic", 0.032f);
        shader.setVec3("pointLight[" + number + "].ambient", 0.3f, 0.3f, 0.3f);
        shader.setVec3("pointLight[" + number + "].diffuse", 0.8f, 0.8f, 0.8f);
        shader.setVec3("pointLight[" + number + "].specular", 1.0f, 1.0f, 1.0f);
    }
}
...
void SetDynamicLights(Shader& shader) {
    ...
    shader.setVec3("spotLight.specular", 0.8f, 0.8f, 0.8f);
...
void Game(GLFWwindow* window, Shader& shader, unsigned int VAO, std::vector<glm::vec3>& cubePositions) {
    if (isDebugMode) {
        ...
        if (isDebugColorMode) {
            ...
            shader.setBool("isDebugColors", isDebugColorMode);
        }
        else {
            shader.use();
            glBindVertexArray(VAO);
